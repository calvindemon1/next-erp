...
export default function JBDeliveryNoteForm() {
  ...
  onMount(async () => {
    ...
    if (isEdit) {
      ...
      const poDetailResponse = await getJualBelis(
        suratJalanData.jb_id,
        user?.token
      );
  ...
  const generateSJNumber = async (salesType, ppn) => {
    const lastSeq = await getLastSequence(user?.token, "jb_sj", salesType, ppn);
  ...
  const addItemGroup = () => {
    ...
    if (!poDetail || !poDetail.items || poDetail.items.length === 0) {
      Swal.fire(
        "Peringatan",
        "Silakan pilih Jual Beli terlebih dahulu.",
        "warning"
      );
      return;
    }
  ...
  return (
    <MainLayout>
      ...
      <h1 class="text-2xl font-bold mb-4">
        {isView ? "Detail" : isEdit ? "Edit" : "Tambah"} Surat Penerimaan Jual
        Beli
      </h1>
      <button
        type="button"

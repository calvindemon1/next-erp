...
export default function BanksList() {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await softDeleteBank(id, tokUser?.token);
  ...
  const handleGetAllBanks = async (tok) => {
    const getDataBanks = await Banks.getAll(tok);
    console.log(tok)

    if (getDataBanks.status === 200) {
      const sortedData = getDataBanks.bank.sort((a, b) => a.id - b.id);
      setBanks(sortedData);
    }
  };
  ...
  createEffect(() => {
    if (tokUser?.token) {
      handleGetAllBanks(tokUser?.token);
    }
  });
          ...
          <tbody>
            {paginatedData().map((bank, index) => (
              <tr class="border-b" key={bank.id}>
                ...
                <td class="py-2 px-4">{bank.kode}</td>
                <td class="py-2 px-4">{bank.nama}</td>

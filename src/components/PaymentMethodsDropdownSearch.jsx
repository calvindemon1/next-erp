...
export default function PaymentMethodsDropdownSearch({
  options,
  paymentMethods,
  form,
  setForm,
  ...
  const list = createMemo(() => {
    ...
    if (typeof paymentMethods === "function") {
      const maybe = paymentMethods();
      return Array.isArray(maybe) ? maybe : [];
    }
  ...
  const selected = createMemo(() =>
    list().find((p) => String(p.id) === String(form()?.[valueKey]))
  );
  ...
  const setFormSafe = (key, val) => {
    try {
      ...
      try {
        ...
        console.error("PaymentMethodsDropdownSearch: setForm failed", err);
      }
    }
  };
